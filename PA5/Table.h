...
//    return tr1::hash<string>()(word) % hashSize;
    ...
    unsigned int hashSize;  // size of the hash table (used in hashCode method above)              
    ListType * tableData;   // tableData of hashTable: array of ListType
    
    // obtain the number of non-empty Bucket 
    int getNonEmptyBucketsNum()const;

    // obtain the number of longest chain items
    int getLongestChainNum()const;
